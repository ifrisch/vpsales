...
try:
    ...
    # --- TABBED DATA SECTION ---
    tab1, tab2 = st.tabs(["🏆 New Customers", "⏲ Pending Customers"])
                ...
                with st.expander(f"**{salesrep}** ({len(group_df)} customers)", expanded=False):
                    for _, row in group_df.iterrows():
                        ...
                        if pd.notna(invoice_date):
                            ...
                            st.markdown(f"• **{row['New Customer']}** - *Invoice: {date_str}*")
                        else:
                            st.markdown(f"• **{row['New Customer']}**")
        else:
    ...
    with tab2:
        ...
        if not df_pending.empty:
            for salesrep, group_df in df_pending.groupby("Salesrep"):
                with st.expander(f"**{salesrep}** ({len(group_df)} customers)", expanded=False):
                    for _, row in group_df.iterrows():
                        st.markdown(f"• **{row['New Customer']}** - *Awaiting first invoice*")
        else:
